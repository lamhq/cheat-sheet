...
- Use methods like `click`, `fill`, `type`, `check` wait for the element to be actionable.  
- Avoid explicit waiting (`waitForSelector`, `waitForLoadState`, `waitForTimeout()`, ` waitUntil: 'networkidle'`).
...
```ts
// ❌ Anti-pattern: manual visibility check before click
...
// ❌ Anti-pattern: no leveraging auto‑waiting at all
