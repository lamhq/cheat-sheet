...
This approach reduces initial page load time, saves bandwidth, improves user experience.


## Implementations

Approaches to implement Lazy loading images: 
...
1. We first render an empty `<img />` element, then use `IntersectionObserver` to detect when the element enters the viewport.
2. When the image is about to appear, the actual `src` is set, triggering the image load.
