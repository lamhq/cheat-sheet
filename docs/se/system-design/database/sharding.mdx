...
Each shard shares the same schema, and stores a portion of the total dataset on a separate database server instance, allowing for parallel processing and reducing the load on any single server.
