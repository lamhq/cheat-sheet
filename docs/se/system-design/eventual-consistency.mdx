...
Services that are eventually consistent are typically described as following [BASE principles](./base-model.mdx), which differ from the traditional [ACID principles](./database/transaction.mdx#acid-properties).
