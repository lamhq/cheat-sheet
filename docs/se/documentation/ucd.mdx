...
A Use Case document describes how users interact with a system to achieve specific goals. 

It captures the sequence of actions (main flow and alternate flows), actors involved, preconditions, and postconditions, ensuring a clear understanding of user interactions and system behavior from the user's perspective.
...
## Overview
This use case describes how an end‑user resets their account password when they cannot log in. It focuses on the user’s actions, expected outcomes, and variations in the process, while ensuring security by not revealing whether an email is registered.
