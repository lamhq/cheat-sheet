...
Provides concrete adapters that implement **Ports** (interfaces) from the **Core** layer.
...
Example:
1. The Application layer (in Core) defines an `OrderRepositoryPort`.
2. The Infrastructure layer provides `PostgresOrderRepository` or `MongoOrderRepository` that implements that port.
...
Repositories are a special type of adapter that encapsulate logic for accessing data in a database.

They are used to decouple database technology from the domain model layer.

Data flow:
1. Repositories receive a domain Entity from the application service.
...
Persistence models are database-oriented representations of data used by repositories to interact with the database.

They are separate from **domain models**. Domain models focus on logic, while persistence models suit database needs.
