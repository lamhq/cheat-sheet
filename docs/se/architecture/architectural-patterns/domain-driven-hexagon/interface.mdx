...
- CLI Controllers
- Event Handlers (of queue, message broker, etc.)

It depends on the **Core** layer, communicating with it via **Commands**/**Queries**, which are handled by **Application Services**. 

Transforms client input into a form that is convenient for **use cases**(services/command handlers) and **entities**.

Transforms output from use cases and entities to a form that is convenient for client (person, external system, etc.). 


## Controllers

Controllers are user-facing APIs that parse requests, trigger business logic, and present results to clients (REST controllers, GraphQL resolvers, message handlers, etc.).
