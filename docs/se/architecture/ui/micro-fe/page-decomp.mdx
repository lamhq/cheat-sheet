...
The web application is divided into distinct Microfrontend apps, each responsible for rendering pages belong to its own feature.

- The decomposition is done at the page level.
- A **shell or container app** loads the correct micro-app based on the route prefix.
- Each micro-app is responsible for its own routing, logic, and rendering for its page.

For example, in a web app with multiple pages:
