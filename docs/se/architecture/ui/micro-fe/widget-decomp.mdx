...
- There's a container application that manages navigation and defines which widgets appear on the UI.
...
With widgets that contain a lots of Javascript, ensure they are packaged in a way that don’t conflict with others (this could end up with a lot of duplication between dependencies and increase page load size).
...
- **Complex orchestration**: Requires a robust shell to manage loading, rendering, and communication.
