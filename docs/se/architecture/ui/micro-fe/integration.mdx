...
3. Version control (monorepo vs polyrepo) shouldn't have any impact on the overall project
4. The shell app can decide to use a specific version (or the latest) of a Microfrontend
5. **Dynamic Runtime Loading**. Microfrontends should be loaded on demand at runtime, avoiding the need to bundle all modules upfront in the host application.
