...
Aggregate is a cluster of domain objects that are treated as a single unit to ensure consistency and enforce business rules.

Aggregate has a root entity, called the Aggregate Root, which acts as the entry point for interacting with the Aggregate. All external access to the Aggregate must go through this root, maintaining control over changes and interactions.

Aggregates **encapsulate** related entities and value objects, ensuring changes within the Aggregate maintain business integrity.

Aggregates enforce rules and constraints to ensure **consistency** within the boundaries of the Aggregate.
